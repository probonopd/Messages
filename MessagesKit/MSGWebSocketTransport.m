...
@interface MSGWebSocketTransport ()
{
	MSGWebSocketState _state;
	NSThread *_thread;
	NSURL *_url;
	CURL *_curl;
	int _sockfd;
...
@implementation MSGWebSocketTransport

- (instancetype)init
{
	self = [super init];
	if (self) {
		_state = MSGWebSocketStateDisconnected;
		_sockfd = -1;
		_wakePipe[0] = -1;
		_wakePipe[1] = -1;
...
{
	if (_state != MSGWebSocketStateDisconnected) {
		return;
	}
	...
	_url = [url retain];
	_state = MSGWebSocketStateConnecting;
	_stop = NO;
...
- (void)runLoop
{
	...
	curl_easy_setopt(_curl, CURLOPT_USERAGENT, "Messages/1.0");
...
{
	_state = MSGWebSocketStateDisconnected;
	NSError *error = [NSError errorWithDomain:@"MSGWebSocketErrorDomain"
		code:code
...
{
	if (_state != MSGWebSocketStateOpen) {
		return;
	}
	[_sendLock lock];
