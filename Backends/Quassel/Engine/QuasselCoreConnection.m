        ...
        if (v.dict) {
            ...
            if ([msgType isEqualToString:@"SessionInit"]) {
                ...
                for (int i = 10; i >= 0; i--) {
                    ...
                    NSString *candidateConfigId = [NSString stringWithFormat:@"%d", i];
                    [self sendCommand:[NSArray arrayWithObjects:[[QVariant alloc] initWithInt:InitRequest],
                                       [[QVariant alloc] initWithString:@"BufferViewConfig"],
                                       [[QVariant alloc] initWithString:candidateConfigId],
                                       nil
                                       ]];
                }
            ...
            if (requestType == Sync) { // 1
                ...
                if ([className isEqualToString:@"BacklogManager"]) {
                    if ([functionName isEqualToString:@"receiveBacklog"]) {
                        ...
                        // Messages: report every reply, including empty and
                        // all-duplicate ones that backlogMessagesReceived: drops,
                        // so the client knows when the buffer's start is reached.
                        if ([delegate respondsToSelector:@selector(quasselBacklogReceivedForBuffer:count:limit:)]) {
                            [delegate quasselBacklogReceivedForBuffer:[[v.list objectAtIndex:4] bufferId]
                                                                count:messageVariantList.count
                                                                limit:[[[v.list objectAtIndex:7] integer] intValue]];
                        }
                    }
    ...
    [messages enumerateObjectsWithOptions:NSEnumerationReverse usingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        ...
        if (!lastSeenForBuffer) {
            ...
            *stop = YES;
            ...
            NSLog(@"computeBufferActivityForBuffer %lu RETURNING,LASTSEEN %@", (unsigned long)idx, message);
            *stop = YES;
            return;
            ...
            *stop = YES;
            ...
            NSLog(@"computeBufferActivityForBuffer %lu RETURNING,CHAT %@", (unsigned long)idx, message);
            *stop = YES;
        } else {
            NSLog(@"computeBufferActivityForBuffer%lu OTHERACTIVITY %@", (unsigned long)idx, message);
