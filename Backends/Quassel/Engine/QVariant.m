...
- (id) initWithSerialization:(NSData*)s  bytesRead:(int*)bytesRead
{
    ...
    if (!s || s.length <= 5) {
        NSLog(@"Reading null serialization (length=%lu)", (unsigned long)s.length);
        return self;
    }
...
- (NSString *)description
{
    ...
    if (dict) {
        ...
        myDescription = [NSMutableString stringWithFormat:@"QVariant(byteArray(%lu))", (unsigned long)byteArray.length];
    else if (message)
