...
static const NSInteger NTNostrSearchIdBase = -100000000;

@interface NTNostermProtocol ()
{
	MSGNetwork *_relayNetwork;
...
@implementation NTNostermProtocol
...
- (void)ensureKeypair
{
	...
	if (key == nil) {
		key = [NTNostrCrypto randomPrivateKey];
		NSLog(@"[NOSTERM] ensureKeypair: generated new random key");
		[self persistKey:key];
	}
	_privateKey = [key retain];
	_pubkeyHex = [[NTNostrCrypto publicKeyXOnlyHexFromPrivateKey:_privateKey] retain];
	NSLog(@"[NOSTERM] ensureKeypair: ready, pubkey=%@", _pubkeyHex);
}

- (NSString *)relayURLString
{
	NTNostrSocketClient *client = _socketClient;
...
// Relays have no IRC channel directory; the known groups stand in for it.
- (MSGCapabilities)capabilities
{
	return MSGCapabilityGroupDirectory | MSGCapabilityHistoryPaging |
		MSGCapabilityClearHistory | MSGCapabilityMute;
}
...
- (NSString *)nostermPublicKeyNpub
{
	...
	return [NTNostrCrypto npubFromPubkeyHex:_pubkeyHex];
}
...
- (NSArray *)knownGroupNames
{
	...
	for (NSNumber *cid in _nip29ChannelIds) {
		MSGChannel *channel = [self.serverState channelWithIdentifier:[cid integerValue]];
...
- (void)flushNameRefresh
{
	...
	[[NSNotificationCenter defaultCenter]
		postNotificationName:MSGNicknamesDidChangeNotification
		object:self];
}
...
- (NSInteger)channelIntIdForNostrId:(NSString *)nostrId
{
	...
	NSInteger identifier = (NSInteger)((uint64_t)self.channelIdBase + (h & NTNostrIdMask));
	if (identifier == 0) {
		identifier = 1;
	}
...
- (void)ensureRelayNetwork
{
	...
	_relayNetwork = [[MSGNetwork alloc] init];
	...
	MSGChannel *lobby = [[MSGChannel alloc] initWithDictionary:@{
		...
		@"name": host,
		@"type": @"lobby",
		@"state": @(MSGChannelStateJoined)
	}];
	[_relayNetwork addChannel:lobby];
	[lobby release];

	[[NSNotificationCenter defaultCenter]
		postNotificationName:MSGNetworkListDidChangeNotification
		object:self];
}

- (void)addLobbyMessage:(NSString *)text
{
	MSGChannel *lobby = [self.serverState channelWithIdentifier:_lobbyChannelId];
	if (lobby == nil || [text length] == 0) {
		return;
	}
	MSGMessage *msg = [[MSGMessage alloc] init];
	msg.identifier = --_lobbyMsgIdCounter;
	msg.channelId = _lobbyChannelId;
	msg.text = text;
	msg.rawText = text;
	msg.timestamp = [NSDate date];
	msg.type = MSGMessageTypeNotice;
...
- (void)publishKind0Metadata
{
	...
	NSDictionary *event = [NTNostrCrypto signedEventWithPubkey:_pubkeyHex
...
- (NSString *)storePath
{
	if (_storePath == nil) {
		...
		_storePath = [[MSGApplicationSupportDirectory() stringByAppendingPathComponent:
...
- (void)handleChannelMessage:(NSDictionary *)event
{
	...
	if ([nostrId length] > 0) {
		MSGMessage *local = _localMessageById[nostrId];
		if (local != nil) {
			local.pending = NO;
	...
	MSGMessage *message = [self messageFromEvent:event channelId:channelId];
	if (message == nil) {
		return;
	}
...
- (MSGMessage *)messageFromEvent:(NSDictionary *)event channelId:(NSInteger)channelId
{
	NSString *nostrId = [event[@"id"] description];
	if ([nostrId length] == 0) {
		return nil;
	...
	MSGMessage *message = [[MSGMessage alloc] initWithDictionary:dict];
...
- (void)handleOk:(NSArray *)args
{
	...
	if ([eventId isEqualToString:_authEventId]) {
		if (accepted) {
			...
			[self.delegate protocol:self didFailWithError:
				[NSError errorWithDomain:@"NTNostermAuthDomain" code:1
	...
	if (accepted) {
		if (eventId != nil) {
			MSGMessage *local = _localMessageById[eventId];
			if (local != nil) {
				local.pending = NO;
		...
		[[MSGLogger sharedLogger] error:@"Nosterm event %@ rejected: %@", eventId, msg];
		if (eventId != nil) {
			MSGMessage *local = _localMessageById[eventId];
			if (local != nil) {
...
- (void)sendJoinForGroupAddress:(NSString *)address
{
	...
	NSString *groupId = [self bareGroupIdForAddress:address];
	NSDictionary *join = [NTNostrCrypto signedEventWithPubkey:_pubkeyHex
		createdAt:(NSUInteger)time(NULL)
		kind:9021
		tags:@[@[@"h", groupId]]
		content:@""
...
- (void)publishGroupDeletionWithEventId:(NSString *)evId address:(NSString *)address
{
	...
	[self ensureKeypair];
	NSDictionary *del = [NTNostrCrypto signedEventWithPubkey:_pubkeyHex
		createdAt:(NSUInteger)time(NULL)
		kind:5
		tags:@[@[@"e", evId]]
		content:@""
	...
	if (address != nil) {
		NSDictionary *dg = [NTNostrCrypto signedEventWithPubkey:_pubkeyHex
			createdAt:(NSUInteger)time(NULL)
			kind:9008
			tags:@[@[@"h", address]]
			content:@""
...
- (void)handleGroupMessage:(NSDictionary *)event
{
	...
	MSGMessage *message = [self groupMessageFromEvent:event channelId:channelId];
	if (message == nil) {
		return;
	}
	[channel addMessage:message];
...
- (MSGMessage *)groupMessageFromEvent:(NSDictionary *)event channelId:(NSInteger)channelId
{
	NSString *nostrId = [event[@"id"] description];
	if ([nostrId length] == 0) {
		return nil;
...
- (void)handleClosed:(NSArray *)args
{
	...
	[[MSGLogger sharedLogger] info:@"Nosterm subscription closed: subId='%@' reason='%@'", subId, msg];
	// A closed subscription means no events will flow for that filter.
	if ([msg length] > 0) {
		[self addLobbyMessage:[NSString stringWithFormat:
			@"*** Subscription '%@' closed: %@", subId, msg]];
		NSError *error = [NSError errorWithDomain:@"NTNostermRelayDomain" code:3
			userInfo:@{NSLocalizedDescriptionKey:
...
- (void)sendMessage:(NSString *)text toChannelId:(NSInteger)channelId
{
	...
	if ([_nip29ChannelIds containsObject:@(channelId)]) {
		...
		if (channel != nil) {
			...
			for (NSUInteger i = start; i < count; i++) {
				MSGMessage *m = (MSGMessage *)msgs[i];
		...
		event = [NTNostrCrypto signedEventWithPubkey:_pubkeyHex
			createdAt:(NSUInteger)time(NULL)
			kind:9
			tags:tags
			content:text
	...
	MSGMessage *message = [[MSGMessage alloc] initWithDictionary:@{
		@"id": @(msgIntId),
		@"msgid": eventId,
		@"type": @"message",
		@"text": text,
...
- (void)sendCommand:(NSString *)command toChannelId:(NSInteger)channelId
{
	...
	if ([verb isEqualToString:@"/join"]) {
		...
		NSInteger lobbyId = 0;
		MSGNetwork *net = [self.serverState networkContainingChannel:channelId];
	...
	if ([verb isEqualToString:@"/part"] || [verb isEqualToString:@"/leave"]) {
		...
		if (intIdNum != nil) {
			...
			MSGChannel *ch = [self.serverState channelWithIdentifier:cid];
			if (ch != nil) {
				[ch setState:MSGChannelStateParted];
			}
		}
		return;
	}
...
- (void)handleGroupMembers:(NSDictionary *)event
{
	...
	MSGChannel *channel = [self.serverState channelWithIdentifier:[intIdNum integerValue]];
	if (channel == nil) {
		return;
	}
	...
	query:(NSString *)query
{
	...
	if (channel != nil && [channel.messages count] > 0) {
		MSGMessage *oldest = [channel.messages firstObject];
...
- (void)handleSearchEvent:(NSDictionary *)event
{
	...
	message.identifier = NTNostrSearchIdBase - idx;
