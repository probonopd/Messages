...
@implementation NTNostrCrypto

static NSData *NTSHA256(NSData *data)
{
...
static NSString *NTHexFromData(NSData *data)
{
...
static NSData *NTDataFromHex(NSString *hex)
{
...
static NSData *NTTaggedHash(NSString *tag, NSData *data)
{
...
static NSData *NTSeria256(BIGNUM *bn)
{
...
static EC_GROUP *NTSecp256k1Group(void)
{
...
{
	EC_GROUP *group = NTSecp256k1Group();
	BIGNUM *n = BN_new();
...
+ (NSString *)publicKeyXOnlyHexFromPrivateKey:(NSData *)sk
{
	...
	NSData *pub = NTSeria256(X);
	NSString *hex = NTHexFromData(pub);
	BN_free(X);
	BN_free(Y);
	EC_POINT_free(P);
	BN_free(priv);
                        ...
                        content:(NSString *)content
{
	...
	return NTHexFromData(NTSHA256(serializedData));
}
...
+ (NSString *)signEventId:(NSString *)eventIdHex privateKey:(NSData *)sk
{
	...
	NSData *msg = NTDataFromHex(eventIdHex);
	if (msg == nil) {
		return nil;
	}
	EC_GROUP *group = NTSecp256k1Group();
	...
	NSData *Pbytes = NTSeria256(X);
	NSData *dbytes = NTSeria256(d);
	...
	NSData *ebytes = NTTaggedHash(@"BIP0340/challenge", challengeInput);
	BIGNUM *e = BN_new();
	...
	NSData *RxOut = NTSeria256(Rx);
	NSData *sOut = NTSeria256(s);
             ...
             message:(NSData *)msg
{
	...
	NSData *sig = NTDataFromHex(sigHex);
	NSData *px = NTDataFromHex(pubkeyHex);
	...
	[ein appendData:px];
	[ein appendData:msg];
	NSData *ebytes = NTTaggedHash(@"BIP0340/challenge", ein);
	BIGNUM *e = BN_new();
	BN_bin2bn(ebytes.bytes, 32, e);
	BN_mod(e, e, n, ctx);
...
static uint32_t NTBech32Polymod(const uint8_t *values, size_t len)
{
...
static void NTBech32HrpExpand(NSString *hrp, uint8_t *out, size_t *outLen)
{
...
static void NTBech32CreateChecksum(NSString *hrp, const uint8_t *data,
	size_t dataLen, uint8_t *out5, size_t *outLen)
{
	uint8_t ext[128];
	size_t extLen = 0;
	NTBech32HrpExpand(hrp, ext, &extLen);
	...
	uint32_t polymod = NTBech32Polymod(values, total) ^ 1;
	for (int i = 0; i < 6; i++) {
...
static int NTBech32ConvertBits(uint8_t *out, size_t *outLen,
...
+ (NSString *)bech32EncodeWithHrp:(NSString *)hrp data:(NSData *)data
{
	...
	size_t convLen = 0;
	if (!NTBech32ConvertBits(conv, &convLen, data.bytes, inLen, 8, 5, 1)) {
		free(conv);
		return nil;
	}
	uint8_t checksum[6];
	size_t csLen = 0;
	NTBech32CreateChecksum(hrp, conv, convLen, checksum, &csLen);
	NSMutableString *s = [NSMutableString stringWithFormat:@"%@1",
		[hrp lowercaseString]];
	for (size_t i = 0; i < convLen; i++) {
		[s appendFormat:@"%c", NT_BECH32_CHARSET[conv[i]]];
	}
	for (int i = 0; i < 6; i++) {
		[s appendFormat:@"%c", NT_BECH32_CHARSET[checksum[i]]];
	}
	free(conv);
	return s;
}
                                 ...
                                 string:(NSString *)string
{
	...
	for (size_t i = 0; i < dataLen; i++) {
		...
		int8_t v = (chv < 128) ? NT_BECH32_CHARSET_REV[chv] : -1;
		if (v < 0) {
			free(conv);
			return nil;
		}
	...
	uint32_t pm = NTBech32Polymod(values, total);
	free(values);
	if (pm != 1) {
		free(conv);
		return nil;
	...
	size_t outLen = 0;
	if (!NTBech32ConvertBits(out, &outLen, conv, payloadLen, 5, 8, 0)) {
		free(conv);
		free(out);
		return nil;
	}
...
{
	NSData *d = NTDataFromHex(pubkeyHex);
