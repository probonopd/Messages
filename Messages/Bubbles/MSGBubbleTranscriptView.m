...
#import "MSGBubbleTranscriptView.h"

#import "MSGBubbleMessage.h"
#import "MSGBubbleTheme.h"
...
@interface MSGBubbleCell : NSObject
{
@public
	MSGBubbleMessage *_message;
...
static NSAttributedString *MSGAttributedFromString(NSString *text,
...
static NSColor *MSGPlaceholderColorForName(NSString *name)
{
	NSUInteger hash = 17;
...
static NSArray *MSGLinkRangesInAttributedString(NSAttributedString *string)
{
...
static NSColor *MSGLerpColor(NSColor *a, NSColor *b, CGFloat t)
{
...
@implementation MSGBubbleTranscriptView
{
...
- (id)initWithFrame:(NSRect)frame theme:(MSGBubbleTheme *)theme
{
	...
	if (self) {
		...
		_theme = [(theme ?: [MSGBubbleTheme defaultTheme]) copy];
...
- (void)setTheme:(MSGBubbleTheme *)theme
{
	if (_theme != theme) {
		[_theme release];
		_theme = [theme copy];
...
- (void)resetCursorRects
{
	...
	for (MSGBubbleCell *cell in _cells) {
		...
		NSArray *ranges = MSGLinkRangesInAttributedString(string);
...
static BOOL MSGMessageShouldBeSkipped(MSGBubbleMessage *message)
{
	if (message == nil) {
		return YES;
	}
...
- (void)addMessage:(MSGBubbleMessage *)message
{
	if (message == nil) {
		return;
	}
	// Skip completely empty messages.
	if (MSGMessageShouldBeSkipped(message)) {
		return;
	}
...
- (void)addMessages:(NSArray *)messages
{
	...
	for (MSGBubbleMessage *message in messages) {
		if (message == nil || MSGMessageShouldBeSkipped(message)) {
			continue;
		}
		[_messages addObject:message];
	}
...
- (void)mouseDown:(NSEvent *)event
{
	...
	for (MSGBubbleCell *cell in [_cells reverseObjectEnumerator]) {
		...
		NSURL *url = [MSGLinkDetector linkAtPoint:textPoint
		                                  inString:
...
- (NSInteger)cellIndexAtPoint:(NSPoint)point
{
	for (NSInteger i = 0; i < (NSInteger)[_cells count]; i++) {
		MSGBubbleCell *cell = [_cells objectAtIndex:i];
...
- (NSRange)selectionRangeInCell:(MSGBubbleCell *)cell atPoint:(NSPoint)point
{
	if (!NSPointInRect(point, cell->_textRect)) {
...
- (NSString *)selectedPlainText
{
	...
	for (NSInteger i = minIdx; i <= maxIdx; i++) {
		...
		MSGBubbleCell *cell = [_cells objectAtIndex:i];
		MSGBubbleMessage *msg = cell->_message;
...
- (NSString *)plainTextLineForMessage:(MSGBubbleMessage *)message
{
	if ([message isDateSeparator]) {
...
- (NSAttributedString *)displayStringForMessage:(MSGBubbleMessage *)message
{
	...
	return MSGAttributedFromString(text, [_theme messageFont], color);
}
...
- (void)appendCellWithMessage:(MSGBubbleMessage *)message
                  typingState:(BOOL)isTyping
                      atTop:(CGFloat)top
                 transcriptWidth:(CGFloat)width
                          result:(MSGBubbleCell **)outCell
                           height:(CGFloat *)outHeight
{
	MSGBubbleTheme *t = _theme;
	...
	MSGBubbleCell *cell = [[MSGBubbleCell alloc] init];
...
static BOOL MSGDateIsDifferentDay(NSDate *a, NSDate *b)
{
...
static NSString *MSGDateSeparatorString(NSDate *date)
{
	if (date == nil) {
		return @"";
	}
...
- (NSDate *)dateForMessage:(MSGBubbleMessage *)message
{
...
- (void)relayout
{
	...
	for (NSUInteger i = 0; i < count; i++) {
		MSGBubbleMessage *message = [_messages objectAtIndex:i];
		BOOL grouped = NO;
		if (i > 0) {
			MSGBubbleMessage *previous = [_messages objectAtIndex:i - 1];
		...
		if (lastDate != nil && !prevWasDateSep && MSGDateIsDifferentDay(lastDate, msgDate)) {
			MSGBubbleMessage *sep = [MSGBubbleMessage dateSeparatorWithText:MSGDateSeparatorString(msgDate)];
	...
	if (_typingVisible) {
		...
		MSGBubbleMessage *typing = [[MSGBubbleMessage alloc] init];
                   ...
                   bottom:(NSColor *)bottomColor
{
	...
	for (NSInteger i = 0; i < steps; i++) {
		...
		NSColor *color = t < 0.5
		    ? MSGLerpColor(topColor, midColor, t * 2.0)
		    : MSGLerpColor(midColor, bottomColor, (t - 0.5) * 2.0);
...
- (void)drawAvatarOfCell:(MSGBubbleCell *)cell
{
	if (NSIsEmptyRect(cell->_avatarRect)) {
		return;
	}
	...
	if (avatar != nil) {
		...
		NSColor *fill = MSGPlaceholderColorForName(
		    cell->_senderName ?: @"?");
		[fill setFill];
		[circle fill];
...
- (void)drawBalloonOfCell:(MSGBubbleCell *)cell
{
	...
	if (cell->_message.senderColor != nil) {
		base = MSGBubbleTintFromUserColor(cell->_message.senderColor);
	}
	...
	[self paintGradientInRect:bubbleRect
	               betweenTop:MSGBubbleLighten(base, 0.42)
	                          mid:base
	                       bottom:MSGBubbleDarken(base, 0.14)];

	NSColor *edge = [MSGBubbleDarken(base, 0.38) colorWithAlphaComponent:0.55];
	[edge setStroke];
...
- (void)drawTypingDotsOfCell:(MSGBubbleCell *)cell
{
	NSRect bubbleRect = cell->_bubbleRect;
...
- (NSRect)paintRectOfCell:(MSGBubbleCell *)cell
{
