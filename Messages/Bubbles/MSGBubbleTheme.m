...
static NSColor *MSGBlendColor(NSColor *color, CGFloat amount)
{
...
@implementation MSGBubbleTheme

+ (instancetype)defaultTheme
{
	static MSGBubbleTheme *theme = nil;
	if (theme == nil) {
		theme = [[MSGBubbleTheme alloc] init];
	}
	return theme;
}
...
{
	MSGBubbleTheme *copy = [[[self class] allocWithZone:zone] init];
...
NSColor *MSGBubbleLighten(NSColor *color, CGFloat amount)
{
	return MSGBlendColor(color, amount);
}

NSColor *MSGBubbleDarken(NSColor *color, CGFloat amount)
{
	return MSGBlendColor(color, -amount);
}

NSColor *MSGBubbleTintFromUserColor(NSColor *userColor)
{
	if (userColor == nil) {
		return nil;
	}
	...
	return MSGBlendColor(userColor, 0.55);
}
