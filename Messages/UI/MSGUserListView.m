...
@interface MSGUserListTableView : NSTableView
{
...
@implementation MSGUserListTableView
...
- (NSMenu *)menuForEvent:(NSEvent *)event
{
	...
	MSGUserListView *owner = (MSGUserListView *)_menuOwner;
...
@implementation MSGUserListView
...
- (instancetype)initWithFrame:(NSRect)frame
{
	...
	if (self) {
		...
		_tableView = [[MSGUserListTableView alloc] initWithFrame:
			NSMakeRect(0, 0, contentSize.width, contentSize.height)];
		[(MSGUserListTableView *)_tableView setMenuOwner:self];
...
- (BOOL)selectUserWithNick:(NSString *)nick
{
	...
	NSInteger row = 0;
	for (MSGUser *user in _channel.sortedUsers) {
...
- (id)tableView:(NSTableView *)tableView objectValueForTableColumn:(NSTableColumn *)tableColumn
	row:(NSInteger)row
{
	MSGUser *user = [_channel.sortedUsers objectAtIndex:(NSUInteger)row];
	return [MSGMessageRenderer attributedStringForNick:user.nick mode:user.mode];
}
