/*
 * Copyright (c) 2026 Simon Peter
 *
 * SPDX-License-Identifier: BSD-2-Clause
 */

#import <AppKit/AppKit.h>

@class MSGMessage;
@class MSGMessageView;
@class MSGBubbleTranscriptView;

@protocol MSGMessageViewDelegate <NSObject>
- (void)messageViewDidScrollToTop:(MSGMessageView *)messageView;
@optional
// Bubble style only: the user clicked a speaker picture in the transcript.
- (void)messageView:(MSGMessageView *)messageView didSelectSenderNick:(NSString *)nick;
@end

// Channel transcript container. Renders either the classic text log or the
// speech-bubble style; switching styles rebuilds the content view empty, so
// callers must repopulate afterwards.
@interface MSGMessageView : NSView
{
	NSScrollView *_scrollView;
	NSTextView *_textView;
	MSGBubbleTranscriptView *_transcriptView;
	NSMutableDictionary *_avatarCache;
	NSMutableSet *_messageIdentifiers;
	NSInteger _channelId;
	BOOL _hasMoreHistory;
	BOOL _usesBubbles;
	id<MSGMessageViewDelegate> _delegate;
}

@property (nonatomic, assign) id<MSGMessageViewDelegate> delegate;
@property (nonatomic, assign) NSInteger channelId;
@property (nonatomic, assign) BOOL hasMoreHistory;

- (void)setUsesBubbles:(BOOL)flag;
- (BOOL)usesBubbles;

- (void)appendMessage:(MSGMessage *)message;
- (void)prependMessages:(NSArray *)messages;
- (void)clear;
- (void)scrollToBottom;

// YES when no messages are currently rendered, i.e. the transcript still shows
// nothing for the selected channel (before its first history page arrives).
- (BOOL)isEmpty;

// NO when the transcript is shorter than the visible area, so there is
// nothing to scroll; callers use this to fetch older messages instead of
// waiting for a scroll-to-top that can never happen.
- (BOOL)contentFillsViewport;

@end
