...
static NSCharacterSet *MSGURLCharSet(void)
{
...
static BOOL MSGIsTrailingPunctuation(unichar c)
{
	switch (c) {
		case (unichar)46: /* . */
		case (unichar)44: /* , */
...
@implementation MSGLinkDetector
...
+ (NSArray<NSValue *> *)rangesOfLinksInString:(NSString *)s
{
	...
	for (NSUInteger i = 0; i < len;) {
		...
		while ((end < len) &&
		       [MSGURLCharSet() characterIsMember:[s characterAtIndex:end]]) {
			end++;
		}
		// Trim trailing sentence punctuation.
		while ((end > start) &&
		       MSGIsTrailingPunctuation([s characterAtIndex:(end - 1)])) {
			end--;
		}
