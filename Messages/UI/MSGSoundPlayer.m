...
static const float MSGMessageSoundGain = 0.3f;

static const NSTimeInterval MSGMessageSoundCooldown = 2.0;
...
static BOOL MSGParseWav(const uint8_t *b, NSUInteger len, MSGWavInfo *out)
{
...
static int32_t MSGClamp(double v, double lo, double hi)
{
	if (v < lo) {
		return (int32_t)lo;
	}
...
static NSData *MSGScaleWav(NSData *wav, float gain)
{
	const uint8_t *src = [wav bytes];
	MSGWavInfo info = {0};

	if (!MSGParseWav(src, [wav length], &info)) {
		return nil;
	}
			...
			switch (info.bitsPerSample) {
				case 8: {
					...
					for (NSUInteger i = 0; i < n; i++) {
						int v = MSGClamp((d[i] - 128) * gain, -128, 127);
						d[i] = (uint8_t)(v + 128);
					}
					break;
				}
				case 16: {
					for (NSUInteger i = 0; i + 1 < n; i += 2) {
						...
						s = (int16_t)MSGClamp(s * gain, -32768, 32767);
						d[i] = (uint8_t)(s & 0xff);
				...
				case 24: {
					for (NSUInteger i = 0; i + 2 < n; i += 3) {
						...
						s = MSGClamp(s * gain, -8388608, 8388607);
						d[i] = (uint8_t)(s & 0xff);
				...
				case 32: {
					for (NSUInteger i = 0; i + 3 < n; i += 4) {
						...
						s = MSGClamp((double)s * gain, -2147483648.0, 2147483647.0);
						d[i] = (uint8_t)(s & 0xff);
						d[i + 1] = (uint8_t)((s >> 8) & 0xff);
						d[i + 2] = (uint8_t)((s >> 16) & 0xff);
...
static float MSGAlertVolumeGain(void)
{
...
+ (void)playMessageSound
{
	...
	if (now - lastPlayed < MSGMessageSoundCooldown) {
		[cooldownLock unlock];
		return;
	}
	lastPlayed = now;
	...
	float gain = MSGMessageSoundGain * MSGAlertVolumeGain();
	NSSound *sound = nil;
	if (gain < 0.99f) {
		NSData *wav = [NSData dataWithContentsOfFile:path];
		NSData *quiet = wav ? MSGScaleWav(wav, gain) : nil;
		if (quiet) {
