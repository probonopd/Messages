...
@interface MSGMessageView () <MSGBubbleTranscriptViewDelegate>
@end

@implementation MSGMessageView
...
- (instancetype)initWithFrame:(NSRect)frame
{
	...
	if (self) {
		...
		_usesBubbles = MSGPreferencesUseBubbles();
		if (_usesBubbles) {
...
{
	NSSize contentSize = [_scrollView contentSize];
	_transcriptView = [[MSGBubbleTranscriptView alloc]
...
- (void)setDelegate:(id<MSGMessageViewDelegate>)delegate
{
	_delegate = delegate;
}

- (id<MSGMessageViewDelegate>)delegate
{
	return _delegate;
}
...
- (NSImage *)avatarForNick:(NSString *)nick
{
	...
	[image lockFocus];
	[[MSGMessageRenderer colorForNick:key] setFill];
	[[NSBezierPath bezierPathWithOvalInRect:
		NSInsetRect(NSMakeRect(0, 0, side, side), 1.0, 1.0)] fill];
	NSString *initial = [MSGMessageRenderer initialForNick:key];
...
- (MSGBubbleMessage *)bubbleMessageForChatMessage:(MSGMessage *)message
{
	MSGBubbleMessage *bubble = [[MSGBubbleMessage alloc] init];
	...
	if (![message isSystemMessage]) {
		bubble.senderColor = [MSGMessageRenderer colorForNick:nick];
	}
	bubble.attributedText =
		[MSGMessageRenderer attributedStringForBubbleBodyOfMessage:message];
...
- (void)appendMessage:(MSGMessage *)message
{
	if (!message) {
		return;
	}
	...
	[[_textView textStorage] appendAttributedString:
		[MSGMessageRenderer attributedStringForMessage:message]];
	[[_textView textStorage] appendAttributedString:
