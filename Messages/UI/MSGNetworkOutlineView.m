...
- (NSMenu *)menuForEvent:(NSEvent *)event
{
	...
	MSGNetworkOutlineView *owner = (MSGNetworkOutlineView *)_menuOwner;
...
@implementation MSGNetworkOutlineView
...
- (instancetype)initWithFrame:(NSRect)frame
{
	...
	if (self) {
		...
		_outlineView = [[MSGChannelOutlineView alloc] initWithFrame:
			NSMakeRect(0, 0, contentSize.width, contentSize.height)];
		[(MSGChannelOutlineView *)_outlineView setMenuOwner:self];
		NSTableColumn *column = [[NSTableColumn alloc] initWithIdentifier:@"channel"];
		[column setWidth:contentSize.width];
		[column setResizingMask:NSTableColumnAutoresizingMask];
		MSGChannelBadgeCell *badgeCell = [[MSGChannelBadgeCell alloc] init];
...
- (void)setServerState:(MSGServerState *)serverState
{
...
- (MSGServerState *)serverState
{
	return _serverState;
}

- (void)setDelegate:(id<MSGNetworkOutlineViewDelegate>)delegate
{
	_delegate = delegate;
}

- (id<MSGNetworkOutlineViewDelegate>)delegate
{
	return _delegate;
}
...
// The visibility rule lives on MSGChannel so it can be unit-tested without the
// outline. Queries (private messages) appear regardless of joined state; the
// lobby is represented by the network header row itself.
- (NSArray *)visibleChannelsForNetwork:(MSGNetwork *)network
{
	NSMutableArray *result = [NSMutableArray array];
	for (MSGChannel *channel in network.channels) {
		if ([channel isVisibleInOutline]) {
			[result addObject:channel];
		}
	}
	[result sortUsingComparator:^NSComparisonResult(MSGChannel *a, MSGChannel *b) {
		BOOL aLobby = (a.type == MSGChannelTypeLobby);
		BOOL bLobby = (b.type == MSGChannelTypeLobby);
		if (aLobby != bLobby) {
...
- (void)reloadData
{
	...
	for (MSGNetwork *network in [self networks]) {
...
- (NSInteger)rowForChannelId:(NSInteger)channelId
{
	for (MSGNetwork *network in [self networks]) {
		...
		for (MSGChannel *channel in [self visibleChannelsForNetwork:network]) {
	...
	forTableColumn:(NSTableColumn *)tableColumn item:(id)item
{
	...
	if ([item isKindOfClass:[MSGNetwork class]]) {
		...
		MSGChannel *channel = item;
		title = channel.name;
		// Surface the client-side unseen count as a red badge on the cell
		// (see MSGChannelBadgeCell). The same badgeCount drives the Dock badge,
...
{
	if ([item isKindOfClass:[MSGNetwork class]]) {
		MSGChannel *lobby = [(MSGNetwork *)item lobby];
