...
- (void)applicationDidFinishLaunching:(NSNotification *)notification
{
	...
	MSGBubbleTranscriptView *transcript =
	    [[MSGBubbleTranscriptView alloc] initWithFrame:
	...
	[messages addObject:[MSGBubbleMessage messageWithText:@"Hi there!"
	                                          senderName:@"alice"
	                                             outgoing:NO]];
	[messages addObject:[MSGBubbleMessage messageWithText:@"Hey! How is it going?"
	                                          senderName:@"steve"
	                                             outgoing:YES]];
	[messages addObject:[MSGBubbleMessage messageWithText:@"Pretty good. "
	                          @"Just trying out the new transcript view."
	                                          senderName:@"alice"
	                                             outgoing:NO]];
	[messages addObject:[MSGBubbleMessage messageWithText:longText
	                                          senderName:@"steve"
	                                             outgoing:YES]];
	...
	[messages addObject:[MSGBubbleMessage messageWithText:nil
	                                          senderName:@"steve"
	                                             outgoing:YES]];
	MSGBubbleMessage *richMessage = [messages lastObject];
	richMessage.attributedText = rich;
	richMessage.avatar = me;
	[rich release];

	[messages addObject:[MSGBubbleMessage messageWithText:@"Nice."
	                                          senderName:@"alice"
	                                             outgoing:NO]];
	[messages addObject:[MSGBubbleMessage messageWithText:@"One more from me."
	                                          senderName:@"steve"
	                                             outgoing:YES]];
	[messages addObject:[MSGBubbleMessage messageWithText:@"Same speaker again, "
	                          @"grouped with a tighter gap."
	                                          senderName:@"steve"
	                                             outgoing:YES]];
	[messages addObject:[MSGBubbleMessage messageWithText:@":)"
	                                          senderName:@"alice"
	                                             outgoing:NO]];

	for (MSGBubbleMessage *m in messages) {
		if ([m outgoing]) {
			m.avatar = me;
		} else {
			m.avatar = alice;
