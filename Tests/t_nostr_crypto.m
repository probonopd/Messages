...
int main(void)
{
	@autoreleasepool {
		...
		NSString *pub3 = [NTNostrCrypto publicKeyXOnlyHexFromPrivateKey:sk3];
		PASS([pub3 isEqualToString:
			@"f9308a019258c31049344f85f89d5229b531c845836f99b08601f113bce036f9"],
			"x-only pubkey of secret 3 matches the BIP-340 value");

		NSData *sk = [NTNostrCrypto randomPrivateKey];
		PASS([sk length] == 32, "random private key is 32 bytes");
		NSString *pub = [NTNostrCrypto publicKeyXOnlyHexFromPrivateKey:sk];
		PASS([pub length] == 64, "derived public key is 64 hex chars");

		NSString *eventId = [NTNostrCrypto eventIdWithPubkey:pub
			createdAt:1700000000 kind:1 tags:@[@[@"e", @"abc"]] content:@"hello"];
		PASS([eventId length] == 64, "event id is 64 hex chars");

		NSString *sig = [NTNostrCrypto signEventId:eventId privateKey:sk];
		PASS([sig length] == 128, "signature is 128 hex chars");

		NSData *msg = TLHexToData(eventId);
		PASS([NTNostrCrypto verifySignature:sig pubkeyXOnlyHex:pub message:msg],
			"signature verifies against its event id");

		NSMutableData *tampered = [msg mutableCopy];
		((unsigned char *)tampered.mutableBytes)[0] ^= 1;
		PASS(![NTNostrCrypto verifySignature:sig pubkeyXOnlyHex:pub message:tampered],
			"tampered message fails verification");
		[tampered release];

		NSData *sk2 = [NTNostrCrypto randomPrivateKey];
		NSString *pub2 = [NTNostrCrypto publicKeyXOnlyHexFromPrivateKey:sk2];
		PASS(![NTNostrCrypto verifySignature:sig pubkeyXOnlyHex:pub2 message:msg],
			"signature fails under a different public key");

		NSDictionary *event = [NTNostrCrypto signedEventWithPubkey:pub
		...
		PASS([NTNostrCrypto verifySignature:event[@"sig"]
		...
		NSString *emptyBech = [NTNostrCrypto bech32EncodeWithHrp:@"a"
			data:[NSData data]];
		PASS([emptyBech isEqualToString:@"a12uel5l"],
			"bech32(a, empty) equals the BIP-173 vector");
		NSData *decodedEmpty = [NTNostrCrypto
