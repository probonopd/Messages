...
// Offline unit test for Nostr nickname resolution in NTNostermProtocol.
//
...
// binary compiles NTNostermProtocol.m in-process, declaring a category
// is enough to call them.
@interface NTNostermProtocol (NicknameTestAccess)
- (void)ensureKeypair;
...
{
	@autoreleasepool {
		MSGServerState *ss = [[MSGServerState alloc] init];
		MSGClientState *cs = [[MSGClientState alloc] init];

		NTNostermProtocol *proto =
			[[NTNostermProtocol alloc] initWithSocketClient:nil
			                                       serverState:ss
			                                      clientState:cs];
		...
		MSGNetwork *net = [[MSGNetwork alloc] init];
		[net setName:@"relay"];
		[net setUuid:@"relay"];
		[ss addNetwork:net];
		MSGChannel *ch = [[MSGChannel alloc] initWithDictionary:@{
			@"id": @(1),
			@"name": @"chan",
			@"type": @"channel",
			@"state": @(1)
		...
		MSGMessage *msg = [proto messageFromEvent:Kind42Event(author, @"hi Alice")
		                                channelId:1];
		PASS(msg != nil, "kind 42 event builds a MSGMessage");
		...
		MSGMessage *msg2 = [proto messageFromEvent:Kind42Event(author3, @"hi")
		                                 channelId:1];
