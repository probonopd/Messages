...
@interface MSGBubbleTranscriptView (TailTesting)
...
@interface MSGMessageView (SenderTesting)
...
@interface RowRecorder : NSObject <MSGUserListViewDelegate, MSGMessageViewDelegate>
@end
...
int main(void)
{
	...
	// --- Preferences -----------------------------------------------------
	{
		BOOL original = MSGPreferencesUseBubbles();

		MSGPreferencesSetUseBubbles(!original);
		PASS(MSGPreferencesUseBubbles() == !original,
			"preference setter roundtrips");

		__block BOOL notified = NO;
		id observer = [[NSNotificationCenter defaultCenter]
			addObserverForName:MSGBubbleStyleDidChangeNotification
			                object:nil
			                 queue:nil
		...
		NSDictionary *saved = MSGPreferencesLastChannelForServer(server);
		PASS(saved == nil, "no stored tab for an unknown server");

		MSGPreferencesSetLastChannelId(42, @"#lounge", server);
		saved = MSGPreferencesLastChannelForServer(server);
		PASS([saved[@"id"] integerValue] == 42 &&
		     [saved[@"name"] isEqual:@"#lounge"],
		    "stored tab keeps id and name");
		MSGPreferencesSetLastChannelId(43, @"#other", server);
		saved = MSGPreferencesLastChannelForServer(server);
		PASS([saved[@"id"] integerValue] == 43 &&
		     [saved[@"name"] isEqual:@"#other"],
		    "storing again replaces the previous tab");
		PASS(MSGPreferencesLastChannelForServer(@"other://server") == nil,
		    "servers do not share their stored tab");
	}

	// --- Bubble messages -------------------------------------------------
	{
		MSGBubbleMessage *m = [MSGBubbleMessage messageWithText:@"hi"
		                                           senderName:@"alice"
		                                              outgoing:NO];
		...
		NSColor *tint = MSGBubbleTintFromUserColor(user);
		CGFloat br = 0.0;
		CGFloat bt = 0.0;
	...
	// --- Transcript view -------------------------------------------------
	{
		MSGBubbleTranscriptView *tv = [[MSGBubbleTranscriptView alloc]
			initWithFrame:NSMakeRect(0, 0, 400, 300) theme:nil];
		PASS([tv messageCount] == 0, "new transcript is empty");

		MSGBubbleMessage *a = [MSGBubbleMessage messageWithText:@"first"
		                                           senderName:@"alice"
		                                              outgoing:NO];
		MSGBubbleMessage *b = [MSGBubbleMessage messageWithText:@"second"
		                                           senderName:@"bob"
		                                              outgoing:YES];
		...
		MSGBubbleMessage *older = [MSGBubbleMessage messageWithText:@"older"
		                                                senderName:@"carol"
		                                                   outgoing:NO];
	...
	// --- Avatar click selects the speaker in the user list ---------------
	{
		MSGChannel *channel = [[MSGChannel alloc] initWithDictionary:@{
			...
			@"messages": @[] }];
		[channel addUser:[[MSGUser alloc] initWithDictionary:
		...
		MSGUserListView *list = [[MSGUserListView alloc]
		...
		MSGMessageView *mv = [[MSGMessageView alloc]
	...
	{
		PASS([[MSGMessageRenderer initialForNick:@"alice"] isEqual:@"A"],
			"initial is the first character of the name");
		PASS([[MSGMessageRenderer initialForNick:@"+bob"] isEqual:@"B"],
			"mode prefixes do not become the initial");
		PASS([[MSGMessageRenderer initialForNick:@""] isEqual:@"?"],
			"empty name falls back to a placeholder");
		NSString *accented = [MSGMessageRenderer initialForNick:@"\u00E9mile"];
	...
	{
		MSGUser *sender = [[MSGUser alloc] init];
		sender.nick = @"alice";
		...
		MSGMessage *chat = [[MSGMessage alloc] init];
		chat.type = MSGMessageTypeMessage;
		chat.sender = sender;
		chat.timestamp = when;
		chat.text = @"hello world";
		NSAttributedString *body =
			[MSGMessageRenderer attributedStringForBubbleBodyOfMessage:chat];
		...
		MSGMessage *action = [[MSGMessage alloc] init];
		action.type = MSGMessageTypeAction;
		action.sender = sender;
		action.timestamp = when;
		action.text = @"waves";
		body = [MSGMessageRenderer attributedStringForBubbleBodyOfMessage:action];
		PASS([body length] > 0 &&
			[[body string] rangeOfString:@"* alice waves"].location != NSNotFound,
			"action renders as '* nick waves'");
		[action release];

		MSGMessage *join = [[MSGMessage alloc] init];
		join.type = MSGMessageTypeJoin;
		join.sender = sender;
		join.timestamp = when;
		join.text = @"alice has joined";
		body = [MSGMessageRenderer attributedStringForBubbleBodyOfMessage:join];
		PASS([body length] > 0 &&
		...
		body = [MSGMessageRenderer attributedStringForBubbleBodyOfMessage:nil];
	...
	// --- Links ------------------------------------------------------------
	{
		NSArray *ranges = [MSGLinkDetector rangesOfLinksInString:
		...
		ranges = [MSGLinkDetector rangesOfLinksInString:
		...
		ranges = [MSGLinkDetector rangesOfLinksInString:@"no links here"];
		...
		// when the second message was rendered after a pool drain).
		{
			...
			[MSGLinkDetector rangesOfLinksInString:@"warmup https://a.example"];
			[inner release];
		}
		ranges = [MSGLinkDetector rangesOfLinksInString:
		...
		ranges = [MSGLinkDetector rangesOfLinksInString:wiki];
		PASS([ranges count] == 1 &&
		...
		NSAttributedString *body =
		    [MSGMessageRenderer attributedStringForMessage:chat];
		...
		NSURL *hit = [MSGLinkDetector linkAtPoint:inside
		...
		hit = [MSGLinkDetector linkAtPoint:below inString:bubbleBody width:300.0];
