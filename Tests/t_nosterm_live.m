...
int main(void)
{
	@autoreleasepool {
		...
		NSString *pubkey = [NTNostrCrypto publicKeyXOnlyHexFromPrivateKey:sk];
		...
		MSGAccount *session = NewNostermAccount(url, @"tl-live-test", key);
		...
		[center addObserver:observer selector:@selector(didFail:)
			name:MSGAccountErrorNotification object:session];
		...
		[center addObserver:observer selector:@selector(usersChanged:)
			name:MSGUserListDidChangeNotification object:session];
		...
		g_usersChanged = NO;
		[session.protocol joinChannelNamed:groupName lobbyId:lobbyId];
		for (MSGNetwork *n in session.serverState.networks) {
		...
		if (channel != nil) {
			...
			while ([[NSDate date] compare:userDeadline] == NSOrderedAscending) {
				for (MSGUser *u in [[channel users] allValues]) {
		...
		if (channel != nil) {
			[session.protocol deleteGroupChannelId:channel.identifier];
