...
int main(void)
{
	...
	/* --- init builds a coherent server state --- */
	{
		...
		MSGClientState *clientState = [[MSGClientState alloc] init];
		...
		MSGNetwork *network = state.networks[0];
		...
		MSGChannel *chan = [network channelWithIdentifier:2];
	...
	/* --- msg event updates channel and posts notification --- */
	{
		...
		id observer = [[NSNotificationCenter defaultCenter]
			addObserverForName:MSGMessagesDidChangeNotification
			object:protocol
			queue:nil
		...
		MSGChannel *chan = [state channelWithIdentifier:2];
	...
	/* --- more event prepends history in order --- */
	{
		...
		MSGMessage *first = chan.messages[0];
	...
	/* --- names populates user list --- */
	{
		...
		MSGUser *alice = [chan userWithNick:@"alice"];
	...
	/* --- topic / nick / channel:state / part events --- */
	{
		...
		PASS([state channelWithIdentifier:2].state == MSGChannelStateParted, "channel state updated");
	...
	/* --- reconnect re-authentication after a disconnect --- */
	{
		...
		MSGServerState *state2 = [[MSGServerState alloc] init];
		MSGClientState *clientState2 = [[MSGClientState alloc] init];
	...
	/* --- server-side search request carries the query --- */
	{
		...
		MSGNetwork *net = [[MSGNetwork alloc] initWithDictionary:
	 ...
	 *     them PARTED --- */
	{
		...
		MSGChannel *query = [network channelWithIdentifier:99];
		PASS(query != nil, "query channel created from join event");
		PASS(query.type == MSGChannelTypeQuery, "query channel has type query");
		PASS(query.state == MSGChannelStateParted, "query channel keeps PARTED state");
		...
		MSGServerState *state2 = [[MSGServerState alloc] init];
		MSGClientState *cs2 = [[MSGClientState alloc] init];
		...
		MSGChannel *q2 = [state2.networks[0] channelWithIdentifier:50];
		PASS(q2 != nil && q2.type == MSGChannelTypeQuery, "query channel from init is created");
		PASS([q2 isVisibleInOutline], "query channel from init is visible");

		// A non-query PARTED channel stays hidden.
		MSGChannel *parted = [[MSGChannel alloc] initWithDictionary:
		...
		MSGChannel *closedQuery = [[MSGChannel alloc] initWithDictionary:
